 ...
 * Class Chatbot
 *
 * Can be loaded using $this->load->controller('api/chatbot');
 *
 * @package Opencart\Catalog\Controller\Api
 */
class Chatbot extends \Opencart\System\Engine\Controller {
	/**
	 * Index - Main chatbot API endpoint
	 *
	 * @return void
	 */
	...
	private function showCategoryProducts(string $message): string {
		...
		foreach ($products as $product) {
			...
			if ($this->customer->isLogged() || !$this->config->get('config_customer_price')) {
				...
				$response .= "💰 Giá: " . $this->currency->format($price_with_tax, $this->config->get('config_currency')) . "\n";
			} else {
	...
	private function getPriceInfo(string $message): string {
		...
		foreach ($products as $product) {
			...
			if ($this->customer->isLogged() || !$this->config->get('config_customer_price')) {
				...
				if ($product['special']) {
					$special_with_tax = $this->tax->calculate($product['special'], $product['tax_class_id'], $this->config->get('config_tax'));
					$response .= "🎯 Giá khuyến mãi: " . $this->currency->format($special_with_tax, $this->config->get('config_currency')) . "\n";
				}
			} else {
				$response .= "💰 Giá: " . $this->currency->format($product['price'], $this->config->get('config_currency')) . "\n";
